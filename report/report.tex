...
\title{Sensing Spaces: Air quality based classification of urban environments and modes of transport using the wearable Airspeck personal exposure monitor}
...
\abstract{
This project is concerned with the use of machine learning techniques to analyse airborne particulates concentration data gathered by wearable Airspeck personal exposure monitors. The aims of the analyses are two-fold:  to classify the route taken by the subjects (mainly in Edinburgh, Scotland) into five categories of urban environments and to identify the subject’s mode of transport as one of walking, cycling, or travelling by bus, train, car or the underground.  The personal Airspeck gathers data on particulate concentrations in terms of PM10, PM2.5 and PM1 values and particulate matter counts in the range of 0.38 to 17 microns which are split into 16 size-resolved bins, as well as temperature and relative humidity. An unsupervised machine learning approach based on the K-means clustering algorithm is used to classify the urban environments. The results compared favourably with the congestion data based on traffic flow speeds provided by Google Maps, as the principal source of pollution was due to vehicular traffic. One of the metrics employed to gauge the quality of the classification  was based on transitional discontinuities between the clustered environments along the routes. A supervised learning approach was considered for the problem of identifying the modes of transport employed by the subject. The best performing model consisting of two different random forest classifiers resulted in a classification accuracy of 95.8\% on a curated training dataset of 5409 inputs based on k-fold cross validation with 5 folds (20\% of the dataset was used for validation). The classification accuracy reduced to 87\% when this method was applied to a dataset collected on a journey from Edinburgh to London with walking, and travel by bus, train and metro. The accuracy was improved to 93\% by employing additional data on speed of movement inferred from the GPS data collected by the Airspeck.
}
...
\begin{figure}[h!]
  \center
  \includegraphics[angle=90, width=1.15\columnwidth]{time_series_london.pdf} 
...
\begin{figure}[h!]
  \center
  \includegraphics[width=\columnwidth]{val_london_conf.pdf}
...
\begin{figure}[h!]
  \center
  \includegraphics[width=\columnwidth]{val_london_conf_speed.pdf}
