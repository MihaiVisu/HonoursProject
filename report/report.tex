...
Moreover, more data was collected by bus several times, on a similar route as the one scanned by \textbf{[NAME]} (Figure \ref{fig:bus_route}) in order to balance the final dataset categories by adding new bus tagged data points.
...
As to the counts of the particulate matter, bin 0 has the highest variance, followed by the next two bins, so the first three bins have been taken into consideration for this analysis. Moreover, exponential moving average (EMA) , which is implemented in the scikit-learn Python library, has been used for normalisation of bin counts, as well as PM1, PM2.5, and PM10 values respectively in order to ease event detection (either change of urban environment or mode of transport) by analysing the means of the attributes at each point. Figures \ref{fig:time_series_meadows} and \ref{fig:time_series_london} present the time series analysis of bin counts and PM values for the Meadows and London journey datasets.
